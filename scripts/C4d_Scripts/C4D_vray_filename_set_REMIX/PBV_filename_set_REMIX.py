...
Requirements:
uj mpappa rendszer +
c4d render beauty nelkul !
uj projekt specifikus nevezek tan +
...
RENDER_SETTINGS = {
    ...
    "DR": {
        ...
        "mp_suffix": "",
        "has_vray": True,
        "vray_suffix": "$prj_$take_$rs_$camera_MP_$frame",
        "render_subfolder": "02_DR\\"
    },
    "HQ": {
        "beauty_suffix": "$prj_$take_$rs_$camera_RGB",
        "has_mp": False,
        "mp_suffix": "",
        "has_vray": True,
        "vray_suffix": "$prj_$take_$rs_$camera_MP_$frame",
        "render_subfolder": "03_HQ\\"
    }
}
...
def set_render_save_path(file_path, render_settings_name):
    ...
    beauty_path = render_folder_path + RENDER_SETTINGS[render_settings_name]["render_subfolder"] + "$prj\\$take\\" + RENDER_SETTINGS[render_settings_name]["beauty_suffix"]
    ...
    if RENDER_SETTINGS[render_settings_name]["has_vray"] and vp is not None:
        vray_path = render_folder_path + RENDER_SETTINGS[render_settings_name]["render_subfolder"] + "$prj\\$take\\" + RENDER_SETTINGS[render_settings_name]["vray_suffix"]
    ...
    if RENDER_SETTINGS[render_settings_name]["has_mp"]:
        mp_path = render_folder_path + RENDER_SETTINGS[render_settings_name]["render_subfolder"] + "$prj\\$take\\" + RENDER_SETTINGS[render_settings_name]["mp_suffix"]
